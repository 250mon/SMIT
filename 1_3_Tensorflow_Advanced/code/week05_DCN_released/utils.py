...
class MnistReader:
    def __init__(self, FLAGS):
        ...
        self.image_file = self._read_file(
            os.path.join(FLAGS.data_path, FLAGS.img_name))
        # if FLAGS.eval_part is 0.9, 90% for training and
        # the rest 10% for evaluation
        # image_size: total number of training images
        self.image_size = int(
            (1. - FLAGS.eval_part) * (self.image_file.seek(0, 2) - 16.) / 784.)

        self.label_file = self._read_file(
            os.path.join(FLAGS.data_path, FLAGS.label_name))
        # label_size: total number of labels
        self.label_size = int((self.label_file.seek(0, 2) - 8.0))

        # the last 10% of the images for evaluation
    ...
    def _get_evaldata(self):
        ...
        # from the 90% position image index to the end
        # label_size - image_size = eval_data size, the number of images for evaluation
        for idx in range(self.image_size, self.label_size):
            self.image_file.seek(16 + 784 * idx, 0)
            self.label_file.seek(8 + idx, 0)
            images.append(
                np.fromfile(self.image_file, dtype=np.ubyte, count=784))
            labels.append(np.fromfile(self.label_file, dtype=np.ubyte,
                                      count=1))
