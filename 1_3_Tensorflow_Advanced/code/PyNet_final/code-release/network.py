...
class Network(object):
    ...
    @layer    
    def _batch_norm(self, tinputs, iCin=10, brst=False, smode='CONV', btrain=True, sname=None):
    ...
    @layer
    def ins_norm(self, tinputs, iCin=10, sname=None):
        ...
        with tf.variable_scope(sname, reuse=tf.AUTO_REUSE):
            ...
            mu, sigma_sq = tf.nn.moments(tinputs, [1,2], keep_dims=True)
    ...
    @layer
    def trconv(self, tinputs, lfilter_shape=(3,3,1,1), istride=2, padding='SAME', buse_bias=False, sinitializer='he_normal', sname=None):
        
        out_shape = tf.multiply(tf.shape(tinputs), (1, istride, istride, 1))
        out_shape = tf.stack((out_shape[0], out_shape[1], out_shape[2], lfilter_shape[2]))
        ...
        with tf.variable_scope(sname, reuse=tf.AUTO_REUSE):
            ...
            if padding == 'REFLECT':
                ...
                pad_size = [[0, 0], [pad1[0],pad2[0]], [pad1[1], pad2[1]], [0,0]]
                inputs = tf.pad(tinputs, pad_size, 'REFLECT')
                
                padding = 'VALID'
                
            x = tf.nn.conv2d_transpose(inputs, kernels, out_shape, strides=istride, padding=padding)
    ...
    @layer
    def trconvact(self, tinputs, lfilter_shape=(3,3,1,1), istride=2, spadding='SAME', buse_bias=False, sinitializer='he_normal', sactivation='ReLu', sname=None):
        
        with tf.variable_scope(sname, reuse=tf.AUTO_REUSE):
            
            (self.feed(tinputs)
                 .trconv(lfilter_shape=lfilter_shape, istride=istride, spadding=spadding, buse_bias=buse_bias, sinitializer=sinitializer, sname=sname)
                 .activation(sactivation=sactivation, sname=sname))
    ...
    @layer
    def convinact(self, tinputs, lfilter_shape=(3,3,1,1), istride=1, spadding='SAME', buse_bias=False, sinitializer='he_normal', sactivation='ReLu', brst=False, btrain=True, sname=None):
        
        with tf.variable_scope(sname, reuse=tf.AUTO_REUSE):
            
            (self.feed(tinputs)
                 .conv(lfilter_shape=lfilter_shape, istride=istride, spadding=spadding, buse_bias=buse_bias, sinitializer=sinitializer, sname=sname)
                 .ins_norm(iCin=lfilter_shape[3], sname=sname+'_in')
                 .activation(sactivation=sactivation, sname=sname))
    ...
    def block_conv(self, tinputs, max_size=3, ch_maps=(1,1), buse_bias=False, sname=None):
        
        with tf.variable_scope(sname, reuse=tf.AUTO_REUSE):
        #convact(self, tinputs, lfilter_shape=(3,3,1,1), istride=1, spadding='SAME', buse_bias=False, sinitializer='he_normal', sactivation='ReLu', sname=None):
            (self.feed(tinputs)
                 .convact(lfilter_shape=(3,3,ch_maps[0], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv3_1')
                 .convact(lfilter_shape=(3,3,ch_maps[1], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv3_2'))
            
            output_tensor = self.terminals[0]
            
            if max_size >= 5:
                (self.feed(tinputs)
                     .convact(lfilter_shape=(5,5,ch_maps[0], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv5_1')
                     .convact(lfilter_shape=(5,5,ch_maps[1], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv5_2'))
                
                output_tensor = tf.concat((output_tensor, self.terminals[0]), axis=3)
                
            if max_size >= 7:
                (self.feed(tinputs)
                     .convact(lfilter_shape=(7,7,ch_maps[0], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv7_1')
                     .convact(lfilter_shape=(7,7,ch_maps[1], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv7_2'))
                
                output_tensor = tf.concat((output_tensor, self.terminals[0]), axis=3)
                
            if max_size >= 9:
                (self.feed(tinputs)
                     .convact(lfilter_shape=(9,9,ch_maps[0], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv9_1')
                     .convact(lfilter_shape=(9,9,ch_maps[1], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv9_2'))
    ...
    def block_convins(self, tinputs, max_size=3, ch_maps=(1,1), buse_bias=False, sname=None):
        
        with tf.variable_scope(sname, reuse=tf.AUTO_REUSE):
        #convact(self, tinputs, lfilter_shape=(3,3,1,1), istride=1, spadding='SAME', buse_bias=False, sinitializer='he_normal', sactivation='ReLu', sname=None):
            (self.feed(tinputs)
                 .convinact(lfilter_shape=(3,3,ch_maps[0], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv3_1')
                 .convinact(lfilter_shape=(3,3,ch_maps[1], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv3_2'))
            
            output_tensor = self.terminals[0]
            
            if max_size >= 5:
                (self.feed(tinputs)
                     .convinact(lfilter_shape=(5,5,ch_maps[0], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv5_1')
                     .convinact(lfilter_shape=(5,5,ch_maps[1], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv5_2'))
                
                output_tensor = tf.concat((output_tensor, self.terminals[0]), axis=3)
                
            if max_size >= 7:
                (self.feed(tinputs)
                     .convinact(lfilter_shape=(7,7,ch_maps[0], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv7_1')
                     .convinact(lfilter_shape=(7,7,ch_maps[1], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv7_2'))
                
                output_tensor = tf.concat((output_tensor, self.terminals[0]), axis=3)
                
            if max_size >= 9:
                (self.feed(tinputs)
                     .convinact(lfilter_shape=(9,9,ch_maps[0], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv9_1')
                     .convinact(lfilter_shape=(9,9,ch_maps[1], ch_maps[1]), istride=1, padding='SAME', buse_bias=buse_bias, sinitializer='he_normal', sactivation='LeakyReLu', sname='conv9_2'))
