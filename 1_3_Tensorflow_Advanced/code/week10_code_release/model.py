...
class VGG16(Network):
    ...
    def __init__(self, cfg):
        ...
        self.batch_img = tf.placeholder(dtype=tf.uint8, name='input') # CIFAR-10 image batch
        self.batch_lab = tf.placeholder(dtype=tf.uint8) # CIFAR-10 label batch
        self.labels = tf.cast(self.batch_lab, tf.int32)
        self.btrain = tf.placeholder(dtype=tf.bool, name='btrain')
        self.breset = tf.placeholder(dtype=tf.bool, name='breset')
        self.buse_drop = tf.placeholder(dtype=tf.bool, name='buse_drop')
        self.learning_rate = tf.placeholder(dtype=tf.float32)
    ...
    def _build(self):
        ...
        ########################################################
        self.reservoir['logits'] = tf.reshape(self._inference(self.batch_img), (-1, 10)) # from Nx1x1x10 to Nx10
        self.reservoir['output'] = tf.argmax(self.reservoir['logits'], axis=1, name='output', output_type=tf.int32) # output node (for excavating graph)
    ...
    def optimizer(self):
        ...
        return train_op, loss_to_opt, self.losses, self.reservoir['logits'], self.summary_vgg, self.reservoir['output']
...
class ResNet34(Network):
    ...
    def __inference(self, tin): #RES34 without BN
    ...
    def _inference(self, tin): #RES34 with BN
        ...
        #resblk(self, tinputs, iCin, iCout, istride=1, buse_bias=False, sinit='he_normal', stype='SHORT', sactivation='ReLu', sname=None):
        (self.feed(inputs)
             .convbnact(lfilter_shape=(3,3,3,64), istride=2, spadding=padding, sinitializer=init, sactivation=act, brst=self.breset, btrain=self.btrain, sname='conv1') #16x16
             #.maxpool(ipool_size=3, istrides=2, spadding=padding, sname='pool1') #16x16
             .resblkbn(64, 64, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn2-1')
             .resblkbn(64, 64, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn2-2')
             .resblkbn(64, 64, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn2-3')
             .resblkbn(64, 128, istride=2, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn3-1') #8x8
             .resblkbn(128, 128, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn3-2')
             .resblkbn(128, 128, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn3-3')
             .resblkbn(128, 128, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn3-4')
             .resblkbn(128, 256, istride=2, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn4-1') #4x4
             .resblkbn(256, 256, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn4-2')
             .resblkbn(256, 256, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn4-3')
             .resblkbn(256, 256, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn4-4')
             .resblkbn(256, 256, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn4-5')
             .resblkbn(256, 256, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn4-6')
             .resblkbn(256, 512, istride=2, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn5-1') #2x2
             .resblkbn(512, 512, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn5-2')
             .resblkbn(512, 512, sinit=init, stype=stype, sactivation=act, brst=self.breset, btrain=self.btrain, sname='resblkbn5-3')
             #dense(iin_nodes=10, iout_nodes=10, btrain=True, bbrst=False, buse_bias=True, buse_bn=False, bbn_first=True, sactivation='ReLu', sinitializer='he_uniform', sname=None):
             .flatten()
             #.dropout(frate=0.2, buse_drop=self.buse_drop, sname='drop1')
             .densebnact(iin_nodes=2048, iout_nodes=512, buse_bias=False, sactivation=act, sinitializer=init, brst=self.breset, btrain=self.btrain, sname='FC1')
    ...
    def optimizer(self):
        ...
        self.saver = tf.train.Saver()
        ckpt_loc = self.cfg.ckpt_dir
        self.ckpt_name = os.path.join(ckpt_loc, 'ResNet34')
