...
class Network(object):
    ...
    @layer
    def resblkbn(self, tinputs, iCin, iCout, istride=1, buse_bias=False, sinit='he_normal', stype='SHORT', sactivation='ReLu', brst=False, btrain=True, sname=None):
        ...
        with tf.variable_scope(sname, reuse=tf.AUTO_REUSE):
            ...
            if stype=='SHORT':
                (self.feed(tinputs)
                     .convbnact(lfilter_shape=(3,3,iCin,iCout), istride=istride, spadding=padding, buse_bias=buse_bias, sinitializer=sinit, sactivation=sactivation, brst=brst, btrain=btrain, sname='conv1')
                     .conv(lfilter_shape=(3,3,iCout,iCout), spadding=padding, buse_bias=buse_bias, sinitializer=sinit, sname='conv2'))
            elif stype=='LONG':
                Cmid = iCout//4
                (self.feed(tinputs)
                     .convbnact(lfilter_shape=(1,1,iCin,Cmid), spadding=padding, buse_bias=buse_bias, sactivation=sactivation,  sinitializer=sinit, brst=brst, btrain=btrain, sname='conv1')
                     .convbnact(lfilter_shape=(3,3,Cmid,Cmid), istride=istride, spadding=padding, buse_bias=buse_bias, sactivation=sactivation,  sinitializer=sinit, brst=brst, btrain=btrain, sname='conv2')
                     .conv(lfilter_shape=(1,1,Cmid,iCout), spadding=padding, buse_bias=buse_bias, sinitializer=sinit, sname='conv3'))
            else:
                raise NotImplementedError('ctype of resblock is not defined in %s'%sname)
